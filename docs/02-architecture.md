...
## 4. KIẾN TRÚC TRIỂN KHAI SẢN PHẨM & CI/CD (VERCEL & GITHUB)

```mermaid
graph LR
    Dev[Developer] -->|git push| GH[GitHub Repository: VuTuanAn1403/English-lms]
    
    subgraph CI/CD Pipeline
        GH -->|Pull Request / Push| GHA[GitHub Actions Workflows]
        GHA -->|Lint & Build| FECI[frontend-ci.yml]
        GHA -->|Maven Test| BECI[backend-ci.yml]
        GHA -->|Secret Scan| SECI[security.yml]
    end
    
    subgraph Frontend Hosting
        GH -->|Deploy Preview / Prod| Vercel[Vercel Serverless Hosting]
        Vercel -->|SPA Client Routing| Browser[User Browser - HTTPS]
    end
    
    subgraph Backend Cloud Infrastructure
        Browser -->|HTTPS /api/*| APIGW[API Gateway - Cloud Host / Docker]
        APIGW --> Microservices[User / Course / AI Microservices]
        Microservices --> PostgreSQL[(PostgreSQL Managed DB)]
        Microservices --> GeminiCloud[Google Gemini AI API]
    end
```
