...
const AdminProfile = () => {
  ...
  const handleChangePassword = async (e) => {
    e.preventDefault();
    if (!oldPassword) {
      setToast({ open: true, message: 'Vui lòng nhập mật khẩu hiện tại!', severity: 'warning' });
      return;
    }
    if (!newPassword || newPassword.length < 6) {
      setToast({ open: true, message: 'Mật khẩu mới phải có tối thiểu 6 ký tự!', severity: 'warning' });
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        ...
        setToast({ open: true, message: res.message || 'Đổi mật khẩu thất bại!', severity: 'error' });
      }
    } catch (err) {
      setToast({ open: true, message: 'Có lỗi xảy ra khi đổi mật khẩu!', severity: 'error' });
    } finally {
      setPwdLoading(false);
    }
  };
