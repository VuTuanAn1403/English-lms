...
class CourseServiceTest {
    ...
    void enroll_Success() {
        ...
        Enrollment enrollment = Enrollment.builder().id(UUID.randomUUID()).course(course).build();
        EnrollmentResponse response = EnrollmentResponse.builder().id(enrollment.getId()).course(courseResponse).build();

        when(courseRepository.findById(courseId)).thenReturn(Optional.of(course));
        when(enrollmentRepository.existsByUserIdAndCourseId(any(UUID.class), eq(courseId))).thenReturn(false);
    ...
    void enroll_AlreadyEnrolled_ThrowsException() {
        ...
        when(courseRepository.findById(courseId)).thenReturn(Optional.of(course));
        when(enrollmentRepository.existsByUserIdAndCourseId(any(UUID.class), eq(courseId))).thenReturn(true);
