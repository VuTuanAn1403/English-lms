...
public class RateLimitingFilter extends OncePerRequestFilter {
    ...
    private static final int MAX_AI_REQUESTS_PER_MINUTE = 30;
    ...
    protected void doFilterInternal(
        ...
        if (path.contains("/api/v1/ai/") && !path.contains("/history")) {
            ...
            if (reqCount.count.get() > MAX_AI_REQUESTS_PER_MINUTE) {
                response.setStatus(HttpStatus.TOO_MANY_REQUESTS.value());
                response.setContentType(MediaType.APPLICATION_JSON_VALUE);
                response.setCharacterEncoding(StandardCharsets.UTF_8.name());
                String body = "{\"success\":false,\"message\":\"Bạn đã gửi quá nhiều yêu cầu tới AI Service. Vui lòng chờ 1 phút.\",\"data\":null}";
